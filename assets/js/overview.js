(function () {
  ...
  function kpiCard({ label, value, unit, delta, sub }) {
    const deltaHtml = delta ? `<div class="kpi__delta ${delta.klass}">${delta.text} ${delta.suffix || ''}</div>` : '';
  ...
  function renderKpis(totals) {
    ...
    const engDelta = totals.engagementPrev > 0
    ...
    grid.appendChild(kpiCard({
      ...
      delta: { ...subsDelta, suffix: `(${(totals.subsDeltaPct * 100).toFixed(2)}%)` },
      sub: 'Sum přes všechny týmy a platformy',
    }));
    grid.appendChild(kpiCard({
      label: `Engagement (${totals.windowDays} d)`,
      value: M.formatNumber(totals.engagement, { compact: true }),
      delta: { ...engDelta, suffix: 'vs minulý' },
      sub: 'Likes + komentáře + sdílení',
    }));
    grid.appendChild(kpiCard({
      label: `Publikováno (${totals.windowDays} d)`,
      value: M.formatNumber(totals.posts),
      delta: { ...postsDelta, suffix: 'vs minulý' },
      sub: `Napříč ${window.ULLH_CONFIG.teams.length} týmy`,
    }));
    const fg = totals.fastestGrowingTeam;
    grid.appendChild(kpiCard({
      label: 'Nejrychlejší růst',
      value: fg && fg.team ? fg.team.short : '—',
      sub: fg ? `${fg.team.name} · ${M.formatDelta(fg.subsDelta).text} followers (${(fg.subsDeltaPct * 100).toFixed(2)}%)` : '',
    }));
  }

  function renderLeaderboard(data) {
    const rows = M.leagueLeaderboard(data, 28);
    ...
    table.innerHTML = `
      <thead>
        <tr>
          ...
          <th style="text-align:right">Postů 28 d</th>
    ...
    rows.forEach((r, i) => {
      ...
      tr.innerHTML = `
        ...
        <td style="text-align:right"><span class="delta ${M.formatDelta(r.subsDelta).klass}">${M.formatDelta(r.subsDelta).text}</span></td>
        <td style="text-align:right">${r.posts}</td>
        <td style="text-align:right">${(r.er * 100).toFixed(2)}%</td>
        <td style="text-align:right"><strong>${(r.score * 100).toFixed(0)}</strong></td>`;
  ...
  function renderCharts(data) {
    C.platformMix(document.getElementById('chart-platform-mix'), M.platformMixByTeam(data));
  ...
  async function init() {
    try {
      ...
      renderMeta(data);
      renderKpis(M.leagueTotals(data, 7));
